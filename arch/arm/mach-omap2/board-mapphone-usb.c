...
/* BEGIN, Motorola, jfbx63, 03/19/12, IKSPYDERICSSKT-164
* implement display wrong model */
#if defined(PRODUCT_SPYDER_SKT)
static char *spyder_dev = "XT910S";
#elif defined(PRODUCT_SPYDER_KT)
static char *spyder_dev = "XT910K";
#else
static char *spyder_dev;
#endif
/* END, IKSPYDERICSSKT-164 */
...
void mapphone_get_product_name(void)
{
	...
	if (strlen(cmdline_modelno)) {
		...
		of_node_put(node);
/* BEGIN, Motorola, jfbx63, 04/03/12, IKSPYDERICSSKT-244
* implement fix wrong model number */
		if (spyder_dev)
			andusb_plat.product_name = (char *)spyder_dev;
/* END, IKSPYDERICSSKT-244 */
	}
